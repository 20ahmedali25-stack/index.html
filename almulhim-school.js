...
function hhOpenSchool(){
  ...
  ov.innerHTML = '<div style="background:#FBF7F0;border:2px solid #B8924A;border-radius:20px;max-width:680px;width:100%;overflow:hidden;margin-bottom:24px;">'
    ...
    +   (function(){
        ...
        if(isTch){
          services += svc('hhOpenClasses()','#1F4E79','#173A5A','صفوفي','إنشاء الصفوف وأكواد الدعوة', I.classes);
          ...
          services += svc('hhOpenCurriculum()','#1F4E79','#173A5A','مركز المناهج','رفع الدروس وتوليد المحتوى', I.curriculum);
        } else {
...
function hhSchLesson(ui, li, kind){
      ...
      thtml = '<div style="background:#FFFDF8;border:1.5px solid #B8924A;border-radius:13px;overflow:hidden;">'
        + terms.map(function(t,i){
            return '<div style="display:grid;grid-template-columns:130px 1fr;'+(i? 'border-top:1px dashed rgba(184,146,74,.35);':'')+'">'
              ...
              +'<div style="padding:9px 12px;color:#4A3A2A;font-size:.78rem;font-weight:700;line-height:1.8;">'+esc(t[1])+'</div></div>';
          }).join('')
        +'</div>';
    } else {
  ...
  // ═══ الملخص وحده (النقاط الرئيسة + القيم) ═══
  var html = '<div style="background:linear-gradient(165deg,#5E0E26,#3D0918);border:1px solid rgba(212,188,133,.55);border-radius:14px;padding:14px 16px 12px;margin-bottom:12px;position:relative;overflow:hidden;">'
    +'<div style="position:absolute;top:0;right:0;left:0;height:3px;background:linear-gradient(90deg,transparent,#D4BC85,transparent);"></div>'
